...
#region CONFIG
    ...
    public class SZConfig
    {
        ...
        public static void InitSZConfig()
        {
            if (!Load())
            {
                Save();
        ...
        private static bool Load()
        {
            try
            {
                if (MyAPIGateway.Utilities.FileExistsInLocalStorage(FILE, typeof(SZConfig)))
                {
                    var file = MyAPIGateway.Utilities.ReadFileInLocalStorage(FILE, typeof(SZConfig));
                    ReadSettings(file);
                    file.Close();
                    return true;
                }
            ...
            {
                SZLogger.Log("Error in Load: " + ex.Message);
            }
            return false;
        }
        ...
        private static void Save()
        {
            try
            {
                var file = MyAPIGateway.Utilities.WriteFileInLocalStorage(FILE, typeof(SZConfig));
...
#region SAFEZONE CAP
    ...
    public class SafeZoneLogic : MyGameLogicComponent
    {
        ...
        {
            if (!Globals.configLoaded) SZConfig.InitSZConfig();
        ...
        {
            //SZLogger.Instance.LogDebug("TerminalControls_CustomControlGetter");
        ...
        public void ChangeValueFloat(string propertyID, float value)
        {
            try
            {
                ...
                if (currentvalue != value){
                    ...
                    SZLogger.Log("SafeZone_PropertiesChanged completed for " + propertyID );
                }
